...
describe('DOCUMENT API', () => {
  ...
  describe('CREATE DOCUMENT POST /documents', () => {
    ...
    (done) => {
      invalidDoc =
      { title: 'the Word', content: 'Lion of the Judah', access: 'strange' };
      superRequest.post('/api/documents')
        .send(invalidDoc)
        .set({ Authorization: adminToken })
        .end((err, res) => {
      ...
      describe('Delete Document DELETE /documents/:id', () => {
        ...
        describe('GET document /documents/:id', () => {
          describe('GET document with PRIVATE access', () => {
              ...
              (done) => {
                superRequest.get(`/api/documents/${privateDocument.id}`)
                  ...
                  .end((err, res) => {
                    ...
                    expect(res.body.document.authorId).to.equal(regularUser.id);
                    done();
                  });
              });
            ...
            it('should NOT return document when user is not the author', (done) => {
              superRequest.get(`/api/documents/${privateDocument.id}`)
                .set({ Authorization: regularToken2 })
                .end((err, res) => {
                  expect(res.status).to.equal(401);
            ...
            describe('ROLE ACCESS DOCUMENT', () => {
              ...
              describe('GET ALL DOCUMENT PAGINATION', () => {
                ...
                it(`should return all documents created by a user irrespective of the
                  access level and every other documents with role or public access with
                  limit set to 4`, (done) => {
              ...
              describe('DOCUMENT SEARCH PAGINATION', () => {
                it('should return search results', (done) => {
                  superRequest.get('/api/search/documents?q=' + publicDocs.content.substr(2, 6))
            .set({ Authorization: regularToken2 })
            .end((err, res) => {
              expect(res.status).to.equal(200);
              res.body.documents.rows.forEach((doc) => {
              ...
              expect(res.body.message).to.equal('The search was successful');
                ...
                it('should allow multiple search terms', (done) => {
                  superRequest.get(`/api/search/documents?q=
                    ${publicDocs.content.substr(2, 6)} ${publicDocs.title.substr(1, 6)}`)
                      .set({
                        'x-access-token': regularToken2
                      })
                      .end((err, res) => {
